...
| `user_fork_requested` | `int` | Fork chat from user message footer — message id to branch before (text pre-filled in composer) |
| `user_edit_requested` | `int` | Begin inline edit on user message — message id |
| `user_edit_submitted` | `int`, `str` | Inline edit submitted — message id and new prompt text |
...
Wired via `_AiChatControllerMixin._init_ai_chat_controller`: `message_submitted` → `_on_ai_message_submitted` (starts `AiChatWorker`, streams reply); `stop_requested` → `_on_ai_chat_stop` (`AiChatWorker.cancel` → `Conversation.interrupt` + `_run_until_done_or_stop` timeout); `assistant_fork_requested` → `_on_assistant_fork_requested` (`AiChatSessionService.fork_session_at_message` + session switch); `user_fork_requested` → `_on_user_fork_requested` (`AiChatSessionService.fork_session_at_user_message` + session switch + composer draft after `transcript_load_finished`); `user_edit_requested` → `_on_user_edit_requested` (`AiChatPanel.begin_inline_edit`); `user_edit_submitted` → `_on_user_edit_submitted` (confirm when later messages exist, `edit_user_message_and_rewind`, `truncate_transcript_after`, resubmit on existing user row); `manage_models_requested` → `_on_open_ai_models_settings`. Connect ``usage_updated`` to ``AiChatPanel.deliver_context_usage_metrics`` (stashes per-turn SDK metrics for ``record_assistant_message``) and ``context_compacted`` refresh to ``deliver_context_usage_refresh`` (not lambdas). ``ChatRunRegistry.research_updated`` → ``deliver_research_update``. Live snapshot refresh: ``MainWindow.schedule_app_context_refresh`` / ``flush_app_context_refresh`` / ``refresh_active_session_snapshot`` (active session only while running). Completed assistant rows show ``aiChatAssistantMessageFooter`` (model/cost + ⋯ menu with Fork chat / Copy message). Worker/thread cleanup uses generation-guarded ``_release_*_thread`` handlers. `AiChatWorker.failed(str, str)` carries `(error, partial_assistant_text)`; the controller persists the full streamed text plus a `---` footer on failure.

### ChatRunRegistry

Source: `services/ai/chat/run_registry.py`

| Signal | Parameters | Description |
|--------|------------|-------------|
| `running_sessions_changed` | *(none)* | Active worker set changed (header badge + history running role) |
| `chunk_received` | `session_id`, `run_generation`, `thinking`, `content` | Streaming assistant deltas |
| `status_changed` | `session_id`, `run_generation`, `status` | SDK activity / compaction status |
| `research_updated` | `session_id`, `run_generation`, `payload` | JSON research progress/findings/stopped |
| `usage_updated` | `session_id`, `run_generation`, `metrics` | Per-turn SDK usage metrics |
| `context_compacted` | `session_id`, `run_generation` | OpenHands condensation completed |
| `assistant_finished` | `session_id`, `run_generation`, `thinking`, `content` | Successful turn |
| `failed` | `session_id`, `run_generation`, `message`, `thinking`, `content` | Worker failure or stop |
| `run_finished` | `session_id`, `run_generation` | Worker thread exited |
