...
| Budgets (under AI) | Per **provider connection** (same groups as Models): **Period** (`No reset`, `Daily`, `Weekly`, `Monthly`, `Yearly`), optional **Soft limit** and **Hard limit** (enable checkbox + USD spin box, min **$0.01**). Rows with no enabled limits are omitted from persistence. Valid edits save immediately to QSettings (`ai/provider_budgets`); invalid edits show `aiBudgetStatusLabel` and do not persist. **Cancel** on the Settings dialog does not revert budget rows (same as Models). Removing a provider on the Models page prunes orphaned budget rows. Enforcement is deferred — limits are stored for future client-side gates. `ui/dialogs/settings/ai_budget/`; `services/ai/ai_budget_config.py`. |
