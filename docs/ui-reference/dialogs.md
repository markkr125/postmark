...
``"History"``, ``"AI"``, ``"Models"``, ``"Budgets"``, ``"Private packages"``, ``"npm"``, ``"JSR"``, or
...
| AI | Overview landing for AI settings (parent tree row). Select **Models** for provider configuration, **Agents** for parallel-run behaviour, or **Budgets** for per-connection spend limits. |
| Agents (under AI) | **Advisory concurrent chat runs** spinbox (1–64, default 10, QSettings `ai/max_concurrent_runs`). When active runs meet or exceed this threshold, the chat composer shows a warning before starting another; sends are not blocked. `ui/dialogs/settings/ai_agents/`; `services/ai/chat/chat_run_limits.py`. |
...
| Budgets (under AI) | Per **provider connection** (same groups as Models): table columns **Provider**, **Period spend**, **Period tokens**, **All-time spend**, **All-time tokens**, **Limits** (reset schedule + caps summary), trailing **gear** button. Column widths are user-resizable and persisted in QSettings (`ui/ai_budget_table_header/v4`); defaults favor a wider **Provider** column and a narrower **Limits** column, and unused viewport width is added to **Provider** without switching columns out of interactive resize mode. Gear opens `aiBudgetConnectionDialog` with **Limits** and **Spend details** tabs (rated rows); unrated connections (e.g. Ollama without model rates) open spend details only. **Spend details** tab: intro explaining table alignment, summary card (**This period** + **All time**), labeled **Model / Tokens / Cost** columns, all-time per-model rows (with turn counts), and estimation footer. Reset schedule: **Daily** (hour), **Weekly** (weekday + hour), **Monthly** / **Yearly** (day + hour; yearly adds month), stored as `period_anchor` `YYYY-MM-DDTHH:MM`. Valid limit edits save immediately to QSettings (`ai/provider_budgets`, schema v3); invalid edits show `aiBudgetStatusLabel`. Spend rollups from `global_spend_summary()`. Soft/hard limits are enforced in the AI assistant (composer banner + send gate; Spend flyout budget card). `ui/dialogs/settings/ai_budget/`; `services/ai/ai_budget_config.py`, `services/ai/budget_period.py`, `services/ai/chat/spend_rollup.py`, `services/ai/chat/budget_status.py`. |
