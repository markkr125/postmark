...
| Budgets (under AI) | Per **provider connection** (same groups as Models): summary strip (**This period** / **All time** spend + **View breakdown**), table columns **Provider**, **Period spend**, **Period tokens**, **All-time spend**, **All-time tokens**, **Limits** (reset schedule + caps summary), trailing **gear** menu. Rated rows: **Limits** (opens `aiBudgetLimitsDialog` with reset period, schedule fields, and USD caps) + **Spend details**. Unrated connections (e.g. Ollama without model rates) show token spend only and **Spend details** in the gear menu. Reset schedule: **Daily** (hour), **Weekly** (weekday + hour), **Monthly** / **Yearly** (day + hour; yearly adds month), stored as `period_anchor` `YYYY-MM-DDTHH:MM`. Valid edits save immediately to QSettings (`ai/provider_budgets`, schema v3); invalid edits show `aiBudgetStatusLabel`. Spend rollups from `global_spend_summary()`. Enforcement deferred. `ui/dialogs/settings/ai_budget/`; `services/ai/ai_budget_config.py`, `services/ai/budget_period.py`, `services/ai/chat/spend_rollup.py`. |
