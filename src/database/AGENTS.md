...
| `AiChatMessageModel` | `ai_chat_messages` | `database/models/ai_chat/model/ai_chat_message_model.py` — transcript rows (`content` answer, separator-packed chronological `thinking` phases, `thinking_duration_seconds` / `post_thinking_duration_seconds` for restored “Thought for Ns” headers on the primary and final follow-up blocks; optional `model_id`, `model_label`, `prompt_tokens`, `completion_tokens`, `reasoning_tokens` per assistant turn; nullable `send_model_id`, `send_mode`, `send_agent_id`, `send_reasoning_effort`, `send_thinking_enabled`, `send_run_context_tokens` per user turn) for search/repaint |
...
`list_assistant_usage_messages`, `allocate_fork_session_title`,
`count_fork_family_sessions`).
