...
| `aiChatStickyTurnPrompt` | `StickyUserPromptOverlay` | Viewport overlay for newest eligible user prompt; deterministic `measure_for_width()` + `apply_geometry()` (manual child layout, not `ChatMessageBubble` row layout); copies anchor `sent_at` and expand/collapse; Show more/less syncs anchor via `_on_sticky_overlay_expanded_changed()`; ≥3px viewport right inset; up to 4px further left than transcript anchor; max height 35% viewport with `aiChatUserMessageFade` when clamped; turn selection uses `_MIN_STICKY_HEIGHT_ESTIMATE_PX` before final height fit; hidden when any later turn user bubble is visible near the viewport top, when the selected anchor is visible near the top, when the assistant row does not intersect the viewport, or when the turn answer scrolled past; each `_sync_sticky_turn_prompt()` rebuilds `_sticky_turn_extents` from live layout; `load_transcript` + `_finish_load_transcript_layout` rebuild turn-pair cache and hook all assistant `layout_height_changed` for sticky resync; `aiChatScrollDown` stays above |
