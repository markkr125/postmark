...
| `aiChatScroll` | `QScrollArea` | Transcript scroll; `_scroll_lock_enabled` defaults **False**; streaming lock: upward user movement detaches (even 1px), range-shrink clamp ignored, real bottom re-arms; turn-boundary `_request_turn_bottom_scroll` anchors to last user bubble; `rangeChanged`, chunk flush, and post-flush `layout_height_changed` call `_queue_stream_follow_passes` (one coalesced 0ms frame pass; 16ms retry only when still off-target or range grew after frame; in-flush layout hook suppressed; chunk flush batches deferred markdown/bubble height); follow target `_stream_follow_target()`; `_turn_scroll_anchor` persists after stream end for turn-start scroll; `_sticky_turn_pairs` + `_sticky_turn_extents` cached with dirty invalidation; scroll/resize/layout hooks call `_schedule_sticky_sync()`; `_sync_sticky_turn_prompt()` picks closest viewport turn into `_sticky_turn_anchor` and shows read-only clone overlay (skips widget mutation when applied anchor/geom/cap unchanged) |
| `aiChatStickyTurnPrompt` | `ChatMessageBubble` | Viewport overlay clone of closest eligible user prompt (`WA_TransparentForMouseEvents`); copies anchor `sent_at`; `prepare_sticky_overlay()` compacts row margins; ≥3px viewport right inset; up to 4px further left than transcript anchor; same `aiChatMessageUser` styling; max height 35% viewport with `aiChatUserMessageFade` when clamped; hidden when real anchor visible or turn answer scrolled past; `load_transcript` + `_finish_load_transcript_layout` rebuild turn-pair cache and hook all assistant `layout_height_changed` for sticky resync; `aiChatScrollDown` stays above |
...
| `aiChatMessageUser` | `QFrame` | Full-width user message bubble (`border-radius: 5px`; child of `ChatMessageBubble`) |
