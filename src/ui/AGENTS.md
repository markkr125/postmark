...
| `aiChatStickyTurnPrompt` | `ChatMessageBubble` | Viewport overlay clone of closest eligible user prompt (`WA_TransparentForMouseEvents`); copies anchor `sent_at` into `aiChatUserMessageTime` when present; max height 35% viewport with `aiChatUserMessageFade` bottom gradient when clamped; hidden when real anchor visible or turn answer scrolled past; `load_transcript` + `_finish_load_transcript_layout` rebuild turn-pair cache and hook all assistant `layout_height_changed` for sticky resync; `aiChatScrollDown` stays above |
...
| `aiChatAssistantText` | `QWidget` (`MarkdownContent`) | Assistant answer: static `QTextDocument` paint (not per-row `QTextBrowser`); incremental `StreamingMarkdownCache` while streaming; full `render_chat_markdown_html` on finalize; re-renders on `ThemeManager.theme_changed`; link clicks via `anchorAt`; context-menu **Copy message** (markdown source); wheel events forward to transcript `QScrollArea` |
