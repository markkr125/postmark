...
| `aiChatSessionTitleBar` | `QWidget` | Conversation title row in flyout chrome (below ``sidebarTitleLabel``, above ``sidebarSeparator``): elided ``aiChatSessionTitle`` left, checkable session-history + new chat ``iconButton``s right; visible only when AI panel is open |
...
| `aiChatStickyTurnPrompt` | `StickyUserPromptOverlay` | Viewport overlay for newest eligible user prompt; deterministic `measure_for_width()` + `apply_geometry()` (manual child layout, not `ChatMessageBubble` row layout); collapsed preview uses `aiChatStickyLabelHost` max-height clamp (like transcript `UserMessageSection`); expanded overflow uses `aiChatStickyScroll`; overlay owns expand/collapse independently of transcript rows; Show more/less triggers `_on_sticky_overlay_expanded_changed()` geometry resync only (does not mutate anchor); ≥3px viewport right inset; up to 4px further left than transcript anchor; collapsed max height 35% viewport (`aiChatUserMessageFade` on preview), expanded 60%; turn selection uses `_MIN_STICKY_HEIGHT_ESTIMATE_PX` before final height fit; hidden when any later turn user bubble is visible near the viewport top, when the selected anchor is visible near the top, when the assistant row does not intersect the viewport, or when the turn answer scrolled past; each `_sync_sticky_turn_prompt()` rebuilds `_sticky_turn_extents` from live layout; `load_transcript` + `_finish_load_transcript_layout` rebuild turn-pair cache and hook all assistant `layout_height_changed` for sticky resync; `aiChatScrollDown` stays above |
...
| `aiChatUserMessageConfig` | `QPushButton` | Curved-arrow message-actions button on user message footer; opens `aiUserMessageActionsPopup` |
...
| `aiSessionHistoryRow` | `QWidget` | One session row (elided title + relative time); ``activeSession`` property when row matches the open chat |
