...
class DraggableTreeWidget(QTreeWidget):
    """Custom QTreeWidget that properly handles drag and drop operations."""
...
class CollectionTree(QWidget):
    """
    Manages the QTreeWidget for collections, including building, context menus,
    and item interactions. Composable into the parent CollectionWidget.
    """
    ...
    def _count_real_children(self, item: QTreeWidgetItem) -> int:
        ...
        for i in range(item.childCount()):
            child = item.child(i)
            if child.data(1, ROLE_PLACEHOLDER) != "placeholder":
                count += 1
        return count
    ...
    def _emit_menu_action(self, action: QAction) -> None:
        """Emit item_action_triggered with the item type, id, and action name."""
        ...
        if action_name == "Rename" and item_type == "folder":
            self._rename_folder(item_id, self._current_item)
            return

        if action_name == "Rename" and item_type == "request":
        ...
        if action_name == "Delete":
            if item_type == "folder":
                ...
                if child_count > 0:
                    msg = f"Are you sure you want to delete this folder?\n\nThis will also delete {child_count} item(s) inside it."
                else:
        ...
        if action_name == "Add folder" and item_type == "folder":
            self.new_collection_requested.emit(item_id)
            return

        if action_name == "Add request" and item_type == "folder":
            self.new_request_requested.emit(item_id)
            return

        # Emit generic event for Open, Copy, etc.
        self.item_action_triggered.emit(item_type, item_id, action_name)

    def _rename_folder(self, item_id, tree_item):
        """
    ...
    def _rename_request(self, item_id, tree_item):
        """
            ...
            if widget:
                ...
                if layout and layout.count() >= 2:
                    ...
                    if name_label:
                        ...
                        line_edit.setStyleSheet("padding-left: 0px; border: 1px solid #3498db;")
                        line_edit.selectAll()
                        ...
                        # Connect signals
                        line_edit.returnPressed.connect(lambda: self._finish_request_rename(tree_item, line_edit, True))
    ...
    def _finish_request_rename(self, tree_item, line_edit, from_return):
        ...
        # ----------------------------------------------------------------
        # Inline-edit finished - persist the new name
        # ----------------------------------------------------------------
    ...
    def _on_item_changed(self, item: QTreeWidgetItem, column: int) -> None:
        ...
        if item.data(1, ROLE_PLACEHOLDER) == "placeholder":
            return
    ...
    def _find_item_by_id(self,
                         ...
                         target_id: int,
                         target_type: str) -> QTreeWidgetItem | None:
        """
        ...
        currentKey = f"{parent.data(1, ROLE_ITEM_TYPE)}:{parent.data(0, ROLE_ITEM_ID)}"
    ...
    def _add_items(self, parent: QTreeWidgetItem, mapping: dict[str, Any]) -> None:
        for _key, value in mapping.items():
            ...
            child = QTreeWidgetItem(
                parent,
                [value["name"] if value.get("type") != "request" else "", value["name"]]
            )


            # ← 4️⃣ Store the ID on the item (folder or request)
    ...
    def _method_color(self, method: str) -> str:
        return _METHOD_COLORS.get(method.upper(), _DEFAULT_COLOR)
    ...
    def add_request(self, new_request: dict, parent_id: int) -> None:
        ...
        try:
            ...
            child = QTreeWidgetItem(
                parent_item,
                [spec["name"] if spec.get("type") != "request" else "", spec["name"]]
            )
